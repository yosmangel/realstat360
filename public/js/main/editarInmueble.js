$(document).ready(function(){
	...
	$('.btn-guardar-internos').bind('click', function(e){ 
		...
		$.ajax({
			...
			data : data,
			beforeSend: function(){
			   	$(".btn-guardar-internos").text('Guardando ...');
			   	$(".btn-guardar-internos").attr('disabled',true);
			},
			complete: function(){
			   	$(".btn-guardar-internos").text('Guardar');
			   	$(".btn-guardar-internos").attr('disabled',false);
			},
			success: function(result) {
			   	scrollTo("#w1");
			   	if(result.code==1){
					$('#responseInternos').show().html(result.mensaje+ '<br>');
				}else{
					$('#responseInternos').hide();
